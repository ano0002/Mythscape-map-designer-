...
class Tile():
    ...
    def __init__(self,
        ...
        self.__tile_by_line = (self.image.get_width()-self.tilemargin[0]*2)//self.__offset_by_tile.x
        
        self.__tile_count = self.__tile_by_line * (self.image.get_height()-self.tilemargin[1]*2)//self.__offset_by_tile.y
        
        self._scaling_factor = scaling_factor if scaling_factor else self.default_scaling_factor
    ...
    def draw(self, surface, offset=(0,0)):
        blit_pos = self.pos.elementwise()*self.tilesize.elementwise()*self.scaling_factor+offset
        if self.x <= -1 or blit_pos[0] > surface.get_width():
            return
        if self.y <= -1 or blit_pos[1] > surface.get_height():
            return
        if self.index == -1:
            pygame.draw.rect(surface, self.color, pygame.Rect(blit_pos,self.tilesize.elementwise()*self.scaling_factor))
        else:
    ...
    def __update_rect(self):
        ...
        pos_on_tileset = Vec2(self.index%self.__tile_by_line, self.index//self.__tile_by_line)
        
        top_left = (self.tilemargin + self.__offset_by_tile.elementwise() * pos_on_tileset.elementwise()) * self.scaling_factor
...
if __name__ == "__main__":
    ...
    tileset = pygame.image.load("assets\\Universal\\Universal-Buildings-and-walls.png")
    tiles = [Tile(pos=Vec2(i,j),index=random.choice((-1,i+j*10)),image=tileset,scaling_factor=scale) for i in range(round((screen.get_width()/16+.5)*(1/scale))) for j in range(round((screen.get_height()/16+.5)*(1/scale)))]
